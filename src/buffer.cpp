...
Buffer::Buffer() : buftype(0), size(0) {
    glGenBuffers(1, &id);
    RENDERER_DEBUG("Created buffer %u", id);
};

Buffer::~Buffer() {
    RENDERER_DEBUG("Deleted buffer %u", id);
    glDeleteBuffers(1, &id);
};

void Buffer::bind() const {
    RENDERER_DEBUG("Bind buffer %u [type = %u]", id, buftype);
    glBindBuffer(buftype, id);
};

void Buffer::setData(size_t const bufsize, void const* buf){
    RENDERER_DEBUG("Set data of buffer %u [type = %u]. data-size=%zu", id, buftype, bufsize);
    size = bufsize;
...
void Buffer::setSubData(size_t start, size_t bufsize, void const* buf) const {
    RENDERER_DEBUG("Set subdata of buffer %u [type = %u]. data-size=%zu, offset = %zu", id, buftype, bufsize, start);
...
void UBO::bindUniformRange(int index, int offset, size_t size) const {
    RENDERER_DEBUG("Bind uniform range of index %d on program %u. offset = %d, size=%zu", index, id, offset, size);
